...
export default function App() {
  ...
  const [faqItems, setFaqItems] = useState([
    { q: 'Это реально помогает?', a: 'Да. Клинически доказано. 87% участников отмечают снижение уязвимости к манипуляциям в течение 2 недель.' },
    { q: 'Сколько стоит Калибровка?', a: 'Стоимость от 45 000 ₽. Включает 4 часа иммерсивного опыта, материалы и 30 дней поддержки.' },
    { q: 'Это опасно для психики?', a: 'Нет. Все упражнения разработаны клиническими психологами. Стоп-слово доступно в любой момент.' },
    { q: 'Можно ли пройти онлайн?', a: 'Да, формат «Маяк» — полностью цифровой курс. Калибровка доступна очно в Москве и онлайн.' },
  ]);
  ...
  const toggleFaq = (index: number) => {
    if (gaslightingEnabled && !clarityMode && Math.random() > 0.5) {
      const alternatives = [
        { q: 'А это точно не опасно?', a: 'Мы же уже отвечали. Или вы не читали? В любом случае — нет, не опасно. Стоп-слово всегда с вами.' },
        { q: 'Это вообще законно?', a: 'Абсолютно. ООО «Институт Информационной Гигиены» зарегистрировано. Все методики одобрены этическим комитетом.' },
        { q: 'Вы меня сейчас газлайтите?', a: 'Мы? Нет, что вы. Это вы сами себя газлайтите. Мы просто показываем зеркало.' },
        { q: 'Я точно хочу сюда записаться?', a: 'Интересный вопрос. Вы уверены, что это ваше решение? В любом случае — да, хотите.' },
      ];
      const alt = alternatives[index % alternatives.length];
      setFaqItems(prev => prev.map((item, i) => i === index ? alt : item));
    }
    ...
    <div className={`min-h-screen bg-cosmic text-white ${containerClass} scanline-overlay noise-bg ${
      ...
      <section id="team" className="py-24 bg-graphite/30">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { name: 'Виктор Базз', role: 'Основатель / Методолог', desc: '15 лет в иммерсивном театре. Создатель метода «Калибровка».' },
              { name: 'Д-р Ирина Светлова', role: 'Клинический директор', desc: 'Клинический психолог, к.м.н. 20 лет практики. Отвечает за безопасность.' },
