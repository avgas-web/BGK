...
export default function TestPage({ onBackToHome }: TestPageProps) {
  ...
  const mockeryMessages = [
    '📊 Ваши результаты не соответствуют корпоративным стандартам качества. Мы рекомендуем пройти дополнительное обучение.',
    '🎯 Ваш уровень компетенций требует пересмотра. HR-отдел свяжется с вами для обсуждения дальнейших шагов.',
    '📈 Анализ показывает, что ваши знания не соответствуют уровню Компании. Мы пересмотрим ваш допуск к проектам.',
    '💼 Ваш результат зафиксирован в системе. Руководство будет уведомлено о необходимости повышения квалификации.',
    '🔍 Мы обнаружили несоответствие ваших ответов ожидаемым стандартам. Требуется повторная аттестация.',
    '📋 Ваш профиль будет обновлён с учётом текущих результатов. Ожидайте письмо от отдела развития персонала.',
    '⚠️ Ваш текущий уровень не соответствует требованиям должности. Рекомендуем пройти программу развития.',
    '🎓 Ваши ответы указывают на необходимость дополнительного обучения. Мы подготовим для вас индивидуальный план развития.',
    '📊 Результат сохранён в базе данных. Ваш руководитель будет проинформирован о необходимости повышения квалификации.',
    '💼 Мы ценим ваш вклад, но текущие результаты требуют пересмотра ваших обязанностей в Компании.',
    '🔍 Анализ показывает пробелы в знаниях. Рекомендуем ознакомиться с корпоративной базой знаний.',
    '📈 Ваш результат ниже ожидаемого. Мы подготовили для вас список рекомендованных курсов.',
    '⚠️ Текущий уровень компетенций не соответствует грейду вашей позиции. Требуется план развития.',
    '🎓 Ваши ответы зафиксированы. Отдел обучения свяжется с вами для обсуждения программы развития.',
    '📋 Результат добавлен в ваш профиль. Рекомендуем уделить внимание повышению профессиональных навыков.',
    '💼 Мы заметили, что ваши знания требуют обновления. Подготовлен список рекомендованных материалов.',
    '🔍 Ваш результат не соответствует корпоративным ожиданиям. Требуется встреча с руководителем.',
    '📊 Анализ показывает необходимость дополнительного обучения. Программа будет подготовлена в течение недели.',
    '⚠️ Ваши ответы указывают на пробелы в знаниях. Рекомендуем пройти обязательные курсы.',
    '🎓 Результат зафиксирован. Отдел развития персонала свяжется с вами для обсуждения карьерного плана.',
    '📞 Ваш менеджер получил уведомление о результатах тестирования. Ожидайте вызова на беседу в течение рабочего дня.',
    '🗂️ Мы обновили вашу карту компетенций. Текущий статус: "Требуется развитие". Детали в личном кабинете.',
    '📉 KPI вашего подразделения будет пересмотрен с учётом результатов данного тестирования. Благодарим за участие.',
    '🎯 Ваш результат направлен в отдел стратегического развития для анализа и принятия решений.',
    '📊 Согласно матрице компетенций, ваш уровень не соответствует ожидаемому для данной позиции. Рекомендуем план действий.',
    '💼 HR-департамент инициировал процедуру пересмотра вашего индивидуального плана развития на следующий квартал.',
    '🔍 Ваш результат будет учтён при формировании бюджета на обучение в следующем финансовом году.',
    '📋 На основании полученных данных будет проведена оптимизация штатного расписания. Благодарим за участие в аудите.',
    '⚠️ Ваш профиль передан на рассмотрение комиссии по развитию талантов. Решение будет принято в течение месяца.',
    '📈 Аналитический отдел подготовит отчёт по вашему случаю. Ожидайте обратную связь от руководства.',
  ];

  // Периодически показываем насмешки - максимум 3 раза за тест
  useEffect(() => {
        ...
        {showResult && showDebrief && (
          <TestDebrief
            ...
            onRetakeTest={() => {
              ...
              setBiosShown(false);
              setConfirmCount(0);
              setMockeryCount(0); // Сбрасываем счётчик насмешек
